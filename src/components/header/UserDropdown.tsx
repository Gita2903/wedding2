      ...
      >
        <span className="me-3 h-11 w-11 overflow-hidden rounded-full">
          <Image
            width={44}
            height={44}
            src={session?.user?.name ? `https://ui-avatars.com/api/?name=${encodeURIComponent(session.user.name)}&background=ffb6c1&color=fff` : "/images/user/owner.png"}
            alt="User"
            className="h-full w-full object-cover"
          />
        </span>

        <span className="me-1 block text-theme-sm font-medium">{session?.user?.name || "Tamu"}</span>
